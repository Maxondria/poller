defmodule PollerPhxWeb.AuthController do
  ...
  defp login(conn, user) do
    conn
    # Updates the conn's assigns field
    |> assign(:current_user, user)
    # Allows us to put some value in the session cookie on the conn struct
    |> put_session(:user_id, user.id)
    # Generate a new session id and a cookie, that will return this to the browser
  ...
  defp handle_login_response({:error, :bad_password, conn}) do
    conn
    |> put_flash(:error, "Authentication failed, please check that your password is correct.")
    |> render("new.html")
  end

  defp handle_login_response({:error, :bad_email, conn}) do
    conn
    |> put_flash(:error, "Authentication failed, please check that your email is correct.")
    |> render("new.html")
  end
