defmodule Poller.PollServer do
  ...
  def save_votes(poll) do
    ...
    |> Enum.each(fn choice ->
      current_poll_votes = Map.get(poll.votes, choice.id, choice.votes)

      if current_poll_votes != choice.votes do
        Choices.update_choice(choice, %{votes: current_poll_votes})
      end
    end)
  end
